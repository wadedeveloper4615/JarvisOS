...
#bug
#file(TO_NATIVE_PATH ${JARVISOS_SOURCE_DIR} JARVISOS_SOURCE_DIR_NATIVE)
#workaround
set(JARVISOS_SOURCE_DIR_NATIVE ${JARVISOS_SOURCE_DIR})
 if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Windows")
string(REPLACE "/" "\\" JARVISOS_SOURCE_DIR_NATIVE ${JARVISOS_SOURCE_DIR})
endif()

if((NOT CMAKE_C_COMPILER_ID STREQUAL "Clang") AND (NOT SEPARATE_DBG))
    add_compile_flags("-fdebug-prefix-map=\"${JARVISOS_SOURCE_DIR_NATIVE}\"=ReactOS")
endif()
...
if(SEPARATE_DBG)
    ...
    file(MAKE_DIRECTORY ${JARVISOS_BINARY_DIR}/symbols)
    ...
    set(CMAKE_C_LINK_EXECUTABLE
        ...
        "${OBJCOPY} --only-keep-debug <TARGET> ${JARVISOS_BINARY_DIR}/symbols/${SYMBOL_FILE}"
...
elseif(NO_ROSSYM)
    ...
    set(CMAKE_C_LINK_EXECUTABLE
        ...
        "${RSYM} -s ${JARVISOS_SOURCE_DIR} <TARGET> <TARGET>")
    set(CMAKE_CXX_LINK_EXECUTABLE
...
if(CMAKE_VERSION VERSION_LESS 3.4.0)
    ...
    set(CMAKE_ASM_COMPILE_OBJECT "<CMAKE_ASM_COMPILER> ${_compress_debug_sections_flag} -x assembler-with-cpp -o <OBJECT> -I${JARVISOS_SOURCE_DIR}/sdk/include/asm -I${JARVISOS_BINARY_DIR}/sdk/include/asm <FLAGS> <DEFINES> -D__ASM__ -c <SOURCE>")
    ...
    set(CMAKE_ASM_COMPILE_OBJECT "<CMAKE_ASM_COMPILER> ${_compress_debug_sections_flag} -x assembler-with-cpp -o <OBJECT> -I${JARVISOS_SOURCE_DIR}/sdk/include/asm -I${JARVISOS_BINARY_DIR}/sdk/include/asm <INCLUDES> <FLAGS> <DEFINES> -D__ASM__ -c <SOURCE>")
...
function(set_module_type_toolchain MODULE TYPE)
    ...
    if((${TYPE} STREQUAL "kernelmodedriver") OR (${TYPE} STREQUAL "wdmdriver"))
        ...
        #add_linker_script(${MODULE} ${JARVISOS_SOURCE_DIR}/sdk/cmake/init-section.lds)
    endif()
...
function(CreateBootSectorTarget _target_name _asm_file _binary_file _base_address)
    ...
    add_custom_command(
        OUTPUT ${_object_file}
        COMMAND ${CMAKE_ASM_COMPILER} -x assembler-with-cpp -o ${_object_file} -I${JARVISOS_SOURCE_DIR}/sdk/include/asm -I${JARVISOS_BINARY_DIR}/sdk/include/asm ${_includes} ${_defines} -D__ASM__ -c ${_asm_file}
        DEPENDS ${_asm_file})
